...
fn handle_syscall(f: &mut TrapFrame) {
    ...
    match a4 {
        ...
        SYS_READFILE | SYS_WRITEFILE => unsafe {
            // crate::common::println!("handle syscall");
            ...
            // crate::common::println!("filename: {:?}", &filename);

            let buf_ptr = f.a2 as *mut u8;
            let mut buf_len = f.a3 as usize;

            match crate::fs::fs_lookup(&filename) {
                Some(file) => {
                    if a4 == SYS_WRITEFILE {
                        //crate::common::println!("data: {:?}", &file.data);
                        //crate::common::println!("buflen: {:?}", &buf_len);
                        crate::memory::memcpy(&mut file.data as *mut u8, buf_ptr, buf_len);
                        file.size = buf_len;
                        crate::fs::fs_flush();
                    } else {
                        crate::memory::memcpy(buf_ptr, &file.data as *const u8, buf_len);
                    }

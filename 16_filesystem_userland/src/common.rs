...
pub fn user_readfile(filename: &[u8], filename_len: usize, buf: &mut [u8], buf_len: usize) -> usize {
...
pub fn user_writefile(filename: &[u8], filename_len: usize, buf: &[u8], buf_len: usize) {
    let filename_addr = filename.as_ptr() as usize;
    //crate::common::println!("buf: {:?}", &buf);
    //crate::common::println!("buf_len: {:?}", &buf_len);
    let buf_addr = buf.as_ptr() as usize;
    syscall(SYS_WRITEFILE, filename_addr, filename_len, buf_addr, buf_len);
}
